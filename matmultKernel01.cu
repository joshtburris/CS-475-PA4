...
__global__ void MatMulKernel(Matrix A, Matrix B, Matrix C) {
  ...
  Csub = &C.elements[dim * BLOCK_SIZE * row + BLOCK_SIZE * col];
  ...
  for (int m = 0; m < (A.width / BLOCK_SIZE); ++m) {
    ...
    Asub = &A.elements[dim * BLOCK_SIZE * row + BLOCK_SIZE * m];
    Bsub = &B.elements[dim * BLOCK_SIZE * m + BLOCK_SIZE * col];
    ...
    int i = m * BLOCK_SIZE + thread_row;
    int j = m * BLOCK_SIZE + thread_col;
    
    shared_A[thread_row][thread_col] = Asub[row * dim + col];//row * dim + (m * BLOCK_SIZE + thread_col)];
    shared_B[thread_row][thread_col] = Bsub[row * dim + col];//(m * BLOCK_SIZE + thread_row) * dim + col];
